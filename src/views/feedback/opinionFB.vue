<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="地图区域" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="反馈类型" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.feedbackClass }}</span>
        </template>
      </el-table-column>
      <el-table-column label="具体描述" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" width="130" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button size="mini" type="danger" @click="handleDelete(row.backId)">
            删除
          </el-button>
        </template>
      </el-table-column>
...
<script>
  import { getList, exportFlie, deleteUser } from '@/api/feedback/opinionFB'
  ...
  export default {
    ...
    methods: {
      ...
      handleSelectionChange(v) {
        this.deleteUsers = v.reduce((total, curr) => total + curr.backId + ',', '')
      },
      resetForm() {
        this.userForm = {
          id: undefined,
